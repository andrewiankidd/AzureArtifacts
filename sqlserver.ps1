...
write-output "Downloading SQLServer to $env:temp"
Write-Output "------------------------------"

if (!(Test-Path("C:\Windows\Temp\SQLFULL_x64_ENU_Core.box")))
{
	...
	$webClient.DownloadFile($downloadURL, "C:\Windows\Temp\SQLFULL_x64_ENU_Core.box")
}

if (!(Test-Path("C:\Windows\Temp\SQLFULL_x64_ENU_Lang.box")))
{
	...
	$webClient.DownloadFile($downloadURL, "C:\Windows\Temp\SQLFULL_x64_ENU_Lang.box")
}

if (!(Test-Path("C:\Windows\Temp\SQLFULL_x64_ENU_Install.exe")))
{
	...
	$webClient.DownloadFile($downloadURL, "C:\Windows\Temp\SQLFULL_x64_ENU_Install.exe")
}

Write-Output "Installing SQLServer";
if (!(Test-Path("C:\Windows\Temp\SQLFULL_x64_ENU\")))
{
	Start-Process "C:\Windows\Temp\SQLFULL_x64_ENU_Install.exe" 
}

Start-Process "C:\Windows\Temp\SQLFULL_x64_ENU\SETUP.exe" -ArgumentList "/Q", "/ACTION=install", "/IACCEPTSQLSERVERLICENSETERMS=1", "/FEATURES=SQL,MDS,Tools", "/SQLSYSADMINACCOUNTS=$hostname\$adminUser", "/INSTANCENAME=$hostname\mssqlserver" -Wait -NoNewWindow
