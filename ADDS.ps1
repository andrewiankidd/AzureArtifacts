...
if ($deployIndex -eq 1) {
	...
	Install-ADDSDomainController -CreateDnsDelegation:$false -DatabasePath "F:\NTDS" -DomainName "$domainname" -NoGlobalCatalog:$false -InstallDns:$false -LogPath "F:\NTDS" -SiteName "$netBiosName" -SysvolPath "F:\SYSVOL" -NoRebootOnCompletion:$true -Credential $credStore -SafeModeAdministratorPassword $securePassword -Force:$true
	Restart-Computer -Force;
}
